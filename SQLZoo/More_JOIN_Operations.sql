...
WHERE ord != 1 and actorid = (SELECT id FROM actor
WHERE name = "Harrison Ford"));
...
SELECT name FROM actor
WHERE id IN (SELECT actorid FROM (SELECT actorid, count(actorid) AS startingroles FROM casting
WHERE ord = 1 GROUP BY actorid)subquery
WHERE startingroles >= 30);
